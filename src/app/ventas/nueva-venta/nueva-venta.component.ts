...
import { agregarProductos, detalleDeventas, lotes, ventas } from '../../interfaz/interfas';
...
export class NuevaVentaComponent implements OnInit {
  ...
  async agregarProductosAVenta(){
    ...
    await this.firebase
      .agregarDetalleVeta("", this.detalleVenta)
      .then(() => {
        this.listaVentas()
      })
      .catch((err) => {
        this.firebase.error("Codigo de barras incorrecto")
      });
  }

  async agregarAVenta(){
    this.ventas.claveDeVenta = ""
    ...
    await this.firebase
      ...
      .catch((err) => {
        this.firebase.error("Agregue productos antes de vender")
      });
  }

  async listaVentas(){
    (await this.firebase.obtenerDetalleVenta("")).subscribe((res) => {
      this.axuliar2 = res
